!(function(dh, d3) {
  ...
  var scaleX = function(config) {
    ...
    var scaleX = d3
      ...
      .paddingInner(0.4)
      .paddingOuter(0.2);
    var referenceScaleX = scaleX
      .copy()
      .paddingInner(0.1)
      .paddingOuter(0.05);
    var stripeScaleX = scaleX
      .copy()
      .paddingInner(0)
      .paddingOuter(0);
    var lineScaleX = scaleX
      .copy()
      .paddingInner(1)
      .paddingOuter(0.5);
    ...
    shapes
      ...
      .attr("y", function(d) {
        if (config.autoScaleY) {
          return (
            config.chartHeight -
            (config.chartHeight - Math.abs(config.scaleY(d.value)))
          );
        }
        if (d.value >= 0 || config.reverseY) {
          return (
            config.scaleY(0) -
            Math.abs(config.scaleY(d.value) - config.scaleY(0))
          );
        } else {
  ...
  var stackedBarShapes = function(config) {
    ...
    var stackedBar = config.container
      .select(".stacked-bar-group")
      .selectAll("g.stack")
  ...
  var dotShapes = function(config) {
    ...
    var dotLayers = config.container
      .select(".dot-group")
      .selectAll(".dot-layer")
      .data(dataCut);
    var dotLayersUpdate = dotLayers
      .enter()
      .append("g")
      .merge(dotLayers)
  ...
  var stackedAreaShapes = function(config) {
    ...
    var stackedArea = config.container
      .select(".stacked-area-group")
      .selectAll("g.stack-area")
      .data(d3.stack().keys(keys)(data));
    var area = stackedArea
      .enter()
      .append("g")
